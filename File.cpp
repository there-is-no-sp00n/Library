...
vector <Comic> File::comic_to_vector()
{
	...
	//Comic temp;
	string num, title, publisher, year, author, illustrator, issue;
	int counter = 0;
	comic_input.open("File_Comic.txt");

	while(comic_input >> num >> title >> publisher >> year >> author >> illustrator >> issue)
	{
		Comic temp(title, publisher, year, author, illustrator, issue);
...
void File::comic_to_file(vector <Comic> c_file)
{
	...
	comic_file << "#" << "\t" << left << setw(30) << "Title" << " " << setw(20) << "Issue" << " " <<  setw(20) << "Publisher" << " " << setw(20) << "Year" << " " << setw(20) << "Author" << " " << setw(20) << "Illustrator" << endl;
	comic_file << endl;
	...
	for (int i = 0; i < com_size; i++)
	{
		...
		issue = temp.get_issue();



		comic_file << i+1 << "\t" << left << setw(30) << title << " " << setw(20) << issue << " " << setw(20) << publisher << " " << setw(20) << year << " " << setw(20) << author << " " <<  setw(20) << illustrator << " " << endl; 
